...
LROUTER_PORT_STATS_JSON_FILE = '/var/log/vmware/lrouter_port_stats.json'
...
def generate_lrouter_port_stats_json_file(lrouter_port_stats, enable_log=True):
    result = {}
    result["lrouter_port_stats"] = lrouter_port_stats
    ...
    try:
        if enable_log:
            ...
            with open(LROUTER_PORT_STATS_JSON_FILE, 'a') as f:
                json_data = {}
                current_timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
                json_data["timestamp"] = current_timestamp
                # Try this first to see how it looks like other wise try the next
                json_data["lrouter_port_stats"] = lrouter_port_stats
                # json_data["lrouter_port_stats"] = result
                json.dump(json_data, f, indent=4)
                f.write("\n")
                print("Data dumped to "+LROUTER_PORT_STATS_JSON_FILE)

    except FileNotFoundError:
        # Handle the case where the directory does not exist
        logging.error("Directory doesn't exist to create file {}".format(LROUTER_PORT_STATS_JSON_FILE))
    except Exception as e:
        # Catch any other exceptions
        logging.error("Failed to append to file {}, error: {}".format(LROUTER_PORT_STATS_JSON_FILE, e))
...
def parseargs():
    ...
    parser.add_argument('--enable-log', action="store_true", help='Dump to lrouter_port_stats.json')
...
def main():
    ...
    generate_lrouter_port_stats_json_file(lrouter_port_stats, args.enable_log)
